...
class Engine(object):
    ...
    def _track(self, track, bpm, consonants, start=0.0, early=True):
        ...
        for at, run in runs:
            ...
            # Where the notes fall is settled in `Renderer._note_call`,
            # after this -- the engine plays a whole number of frames and
            # drops the remainder, so a note asked for its own length runs
            # short and a phrase creeps forward. It has to be settled after
            # the consonants are moved, not before: `anticipate` rewrites
            # every length it touches, so a phrase squared up here comes out
            # as crooked as it went in. Measured on a nine-part song: 210 ms
            # of creep untouched, 241 ms squared up before anticipating, and
            # 2.5 ms -- half a frame, the engine's own resolution -- squared
            # up after.
